...
// 添加网络模式断开功能，参考linkhelpLogic.cc的实现
static void _disconnect_wifi_for_bt() {
	// 获取当前网络模式
	net_mode_e current_mode = net::get_mode();
	
	// 如果当前不是AP模式，则断开wifi连接
	if (current_mode == E_NET_MODE_WIFI) {
		LOGD("[btsetting] disconnecting wifi for bluetooth operation");
		// 切换到AP模式以断开wifi连接
		net::change_mode(E_NET_MODE_AP);
	} else if (current_mode == E_NET_MODE_P2P) {
		LOGD("[btsetting] switching from P2P to AP mode for bluetooth operation");
		// 从P2P模式切换到AP模式
		net::change_mode(E_NET_MODE_AP);
	}
}
