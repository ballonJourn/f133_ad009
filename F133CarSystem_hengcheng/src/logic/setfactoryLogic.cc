...
static void facoryWnd() {
    ...
    // **新增：检查自动升级条件**
    bool auto_upgrade_flag = mcu_upgrade::check_auto_upgrade_flag();
    bool has_upgrade_file = mcu_upgrade::has_mcu_upgrade_file();  // 使用uart::query_update_file()
    
    if (auto_upgrade_flag && has_upgrade_file) {
        LOGD("[setFactory] Auto upgrade conditions met (flag + upgrade file exist)");
        
        // 获取升级文件路径并计算hash
        std::string upgrade_file = mcu_upgrade::get_mcu_upgrade_file_path();
        
        if (!upgrade_file.empty()) {
            std::string file_hash = mcu_upgrade::calculate_file_hash(upgrade_file.c_str());
            
            if (!file_hash.empty() && !mcu_upgrade::is_hash_recorded(file_hash)) {
                LOGD("[setFactory] ANew MCU upgrade file detected (hash not recorded), starting auto upgrade");
                ...
                if (uart::entry_update_mode()) {
                    ...
                    // 记录hash值
                    mcu_upgrade::record_hash(file_hash);
                } else {
                    LOGE("[setFactory] AFailed to enter MCU update mode");
                    mTextView12Ptr->setBackgroundPic("/set/icon_upgrade_error.png");
                    mTextView18Ptr->setTextTr("No upgrade files found.");
                    mTextView18Ptr->setTextColor(0xFFE81515);
                }
                ...
                return;
            } else if (!file_hash.empty()) {
                LOGD("[setFactory] AMCU upgrade file already processed (hash recorded), showing info window");
                // hash已记录，显示升级信息窗口但不自动升级
                mpasswordWindowPtr->hideWnd();
                mmcuInfoWindowPtr->showWnd();
                query_mcu_update_file();
                return;
            } else {
                LOGE("[setFactory] A Failed to calculate file hash");
            }
        } else {
            LOGE("[setFactory]  Failed to get upgrade file path");
        }
    }
