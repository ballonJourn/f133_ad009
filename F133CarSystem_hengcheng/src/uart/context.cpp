...
static std::set<_s_power_cb> _s_power_state_set;
...
static void _notify_power_state(bool state) {
	Mutex::Autolock _l(_s_mcu_cb_set_lock);
	for(auto it : _s_power_state_set)
	{
		it(state);
	}
}
...
void add_power_state_cb(_s_power_cb cb){
	Mutex::Autolock _l(_s_mcu_cb_set_lock);
	_s_power_state_set.insert(cb);
}
void remove_power_state_cb(_s_power_cb cb){
	Mutex::Autolock _l(_s_mcu_cb_set_lock);
	_s_power_state_set.erase(cb);
}
...
bool set_power_cr(bool status) {
	uint8_t status_data[1] = {0};
	status_data[0] = status;
	return _send_frame_data(CMD_POWER_CR, status_data, TABLESIZE(status_data));
}

// 查询power按键状态
bool query_power_status() {
	uint8_t data[1] = {0};
	data[0] = 0xff;
	return _send_frame_data(CMD_POWER_STATUS, data, TABLESIZE(data));
}

bool app_turn_off() {
	uint8_t data[1] = {0};
	data[0] = 1;
	return _send_frame_data(CMD_POWER_OFF, data, TABLESIZE(data));
}
